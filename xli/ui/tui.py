...
from textual.widgets import Header, Footer, Button, Static, Input, Collapsible, ProgressBar, Label
...
# NOTE: TuiStreamingHandler exists in xli.core.streaming but nothing here
# wires it to a per-agent widget yet — run_chain() only streams internally,
# so progress bars currently jump 20% -> 100% with no live incremental
# update. Left as a known gap rather than faked; see run_chain() below.
...
#modal-dialog { border: solid $primary; padding: 1 2; background: $panel;
                width: 60; height: auto; max-height: 20; }
#modal-dialog Button { margin-top: 1; margin-right: 1; }
...
class ChoiceModal(ModalScreen[int]):
    """Single-choice picker dialog."""

    BINDINGS = [("escape", "cancel", "Cancel")]

    def __init__(self, question: str, options: List[str], default: int = 0):
        super().__init__()
        self.question = question
...
class InputModal(ModalScreen[str]):
    ...
    def compose(self):
        with Vertical(id="modal-dialog"):
            yield Label(self.prompt, id="modal-question")
            yield Input(value=self.default, id="modal-input")
            yield Button("OK", variant="primary", id="ok")

    def on_input_submitted(self, event: Input.Submitted):
        self.dismiss(event.value or self.default)

    def on_button_pressed(self, event: Button.Pressed):
        self.dismiss(self.query_one("#modal-input", Input).value or self.default)
...
class XliTui(App):
    ...
    async def run_chain(self, task: str):
        ...
        try:
            if self.core is None:
                ...
                self.core = XliCore(self.env, mode=AutonomyMode.from_env())
    ...
    # These now use real modal dialogs (ConfirmModal/ChoiceModal/InputModal)
    # instead of returning the default immediately without ever showing
    # anything to the user — the previous stubs meant an agent chain that
    # tried to ask a question via the UI got silently auto-answered.
    ...
    def display(self, text: str, title: Optional[str] = None):
        ...
        if title:
            ...
            console.print(text)
        self._log(text[:200], title or "OUTPUT")

    async def input(self, prompt: str, default: str = "") -> str:
        return await self.push_screen_wait(InputModal(prompt, default))
    ...
    def open_buffer(self, content: list, name: str = "XLI", filetype: str = "markdown"):
        # No separate buffer widget exists yet in this layout, so route the
        # content into the activity log (visible) instead of silently
        # discarding it as the previous `pass` did.
        text = "\n".join(content) if isinstance(content, list) else str(content)
        self._log(text[:500], name)
    ...
    async def choice(self, question: str, options: list, default=None) -> int:
        return await self.push_screen_wait(ChoiceModal(question, options, default or 0))
